...
func Register(app *bootstrap.App) {
	...
	overviewService := service.NewOverviewService(app.DB, k8sRuntimeService)
	...
	clusters.Use(authMiddleware, authorize, opAudit)
	...
	pods := api.Group("/pods")
	pods.Use(authMiddleware, authorize, opAudit)

...
func Register(app *bootstrap.App, runtimeClient *grpcclient.RuntimeClient) {
	...
	k8sPolicies.GET("/cluster-auth-matrix", k8sScopedPolicyHandler.ClusterAuthMatrix)
	k8sPolicies.GET("/user-cluster-auth", k8sScopedPolicyHandler.UserClusterAuth)
	...
	k8sPolicies.POST("/cluster-grants/batch-delete", k8sScopedPolicyHandler.DeleteClusterGrantsBatch)
	...
	pods.GET("/diagnose", podHandler.Diagnose)
	...
	podsWS.Use(wsAuthMiddleware, authorize, k8sScopeAuthorize)
	...
	deployments.GET("/rollout-status", workloadHandler.DeploymentRolloutStatus)
	...
	projectRoutes.Use(authMiddleware, authorize, opAudit)
	projectRoutes.GET("", projectHandler.List)
	projectRoutes.POST("", projectHandler.Create)
	projectScoped := projectRoutes.Group("/:id", middleware.RequireProjectMemberAccess(projectMemberRepo, app.Logger))
	projectScoped.PUT("", projectHandler.Update)
	projectScoped.DELETE("", projectHandler.Delete)
	projectScoped.GET("/members", projectHandler.ListProjectMembers)
	projectScoped.POST("/members", projectHandler.AddProjectMember)
	projectScoped.PUT("/members/:memberId", projectHandler.UpdateProjectMember)
	projectScoped.DELETE("/members/:memberId", projectHandler.RemoveProjectMember)
	projectScoped.GET("/servers", projectHandler.ListServers)
	projectScoped.POST("/servers", projectHandler.UpsertServer)
	projectScoped.GET("/servers/:serverId", projectHandler.ServerDetail)
	projectScoped.DELETE("/servers/:serverId", projectHandler.DeleteServer)
	projectScoped.POST("/servers/:serverId/exec", projectHandler.ExecServerCommand)
	projectScoped.GET("/server-groups/tree", projectHandler.ListServerGroups)
	projectScoped.POST("/server-groups", projectHandler.UpsertServerGroup)
	projectScoped.PUT("/server-groups/:groupId", projectHandler.UpdateServerGroup)
	projectScoped.DELETE("/server-groups/:groupId", projectHandler.DeleteServerGroup)
	projectScoped.GET("/cloud-accounts", projectHandler.ListCloudAccounts)
	projectScoped.POST("/cloud-accounts", projectHandler.UpsertCloudAccount)
	projectScoped.PUT("/cloud-accounts/:accountId", projectHandler.UpdateCloudAccount)
	projectScoped.DELETE("/cloud-accounts/:accountId", projectHandler.DeleteCloudAccount)
	projectScoped.PUT("/cloud-accounts/:accountId/sync", projectHandler.SyncCloudAccount)
	projectScoped.POST("/servers/import", projectHandler.ImportServers)
	projectScoped.GET("/servers/import-template", projectHandler.ServersImportTemplate)
	projectScoped.GET("/servers/export", projectHandler.ExportServers)
	projectScoped.POST("/servers/test", projectHandler.TestServer)
	projectScoped.POST("/servers/test/batch", projectHandler.BatchTestServers)
	projectScoped.POST("/servers/:serverId/cloud-actions", projectHandler.CloudServerAction)
	projectScoped.POST("/servers/sync", projectHandler.SyncServers)
	projectScoped.GET("/services", projectHandler.ListServices)
	projectScoped.POST("/services", projectHandler.UpsertService)
	projectScoped.DELETE("/services/:serviceId", projectHandler.DeleteService)
	projectScoped.GET("/log-sources", projectHandler.ListLogSources)
	projectScoped.POST("/log-sources", projectHandler.UpsertLogSource)
	projectScoped.DELETE("/log-sources/:logSourceId", projectHandler.DeleteLogSource)
	projectScoped.GET("/agents/list", logAgentHandler.List)
	projectScoped.DELETE("/agents/:agentId", logAgentHandler.Delete)
	projectScoped.POST("/agents/heartbeat-refresh", logAgentHandler.BatchRefreshHeartbeat)
	projectScoped.GET("/agents/status", logAgentHandler.Status)
	projectScoped.POST("/agents/bootstrap", logAgentHandler.Bootstrap)
	projectScoped.POST("/agents/rotate-token", logAgentHandler.RotateToken)
	projectScoped.GET("/agents/discovery", agentDiscoveryHandler.List)
	projectScoped.GET("/logs/stream", projectHandler.StreamLogs)
	projectScoped.GET("/logs/export", projectHandler.ExportLogs)
	projectScoped.GET("/log-files", projectHandler.ListLogFiles)
	projectScoped.GET("/log-units", projectHandler.ListLogUnits)

	projectsWS := api.Group("/projects")
	projectsWS.Use(wsAuthMiddleware, authorize, middleware.RequireProjectMemberAccess(projectMemberRepo, app.Logger))

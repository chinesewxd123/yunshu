...
func Register(app *bootstrap.App) {
	...
	alertService := service.NewAlertService(app.DB, app.Redis, app.Mailer, app.Config.Alert)
	...
	projectMgmtService, err := service.NewProjectMgmtService(projectRepo, serverRepo, serviceRepo, logRepo, app.Config.Security.EncryptionKey)
	if err != nil {
		panic(err)
	}
	...
	projectHandler := handler.NewProjectHandler(projectMgmtService)
	logAgentHandler := handler.NewLogAgentHandler(logAgentService)
	agentDiscoveryHandler := handler.NewAgentDiscoveryHandler(agentDiscoveryService)
	...
	api.GET("/agents/ws/ingest", logAgentHandler.IngestWS)

...
import { Alert, Button, Dropdown, Form, Input, Modal, Progress, Space, Switch, Tag, Typography, message } from "antd";
...
import {
  ...
  listCronJobPods,
  listCronJobsV2,
  patchCronJobContainerResources,
  suspendCronJob,
  triggerCronJob,
  type CronJobItemV2,
  type WorkloadDetail,
...
`}
      ...
      <Modal
        title={`CronJob 垂直扩缩（jobTemplate resources · VPA 类）${verticalTarget ? `：${verticalTarget.name}` : ""}`}
        open={verticalOpen}
        ...
        onOk={() => {
          ...
          void verticalForm.validateFields().then(async (values) => {
            ...
            await patchCronJobContainerResources(verticalTarget.clusterId, verticalTarget.namespace, verticalTarget.name, {
      ...
      >
        <Alert
          ...
          message="CronJob 无 HPA 式水平副本扩缩；修改 jobTemplate 内 resources 主要影响后续调度创建的 Job。若使用 VPA，Job/CronJob 场景常在 Initial / Off 模式下更安全。"
        />

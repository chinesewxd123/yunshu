...
import { LoginOutlined, HistoryOutlined } from "@ant-design/icons";
...
function ensureLogMenus(items: AntdMenuItem[]): AntdMenuItem[] {
  ...
  const hasAllLogs = LOG_MENU_KEYS.forEach((key) => allKeys.has(key));
  if ([...LOG_MENU_KEYS].every((key) => allKeys.has(key))) {
    return items;
  }

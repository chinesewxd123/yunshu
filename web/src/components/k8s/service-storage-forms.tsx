  ...
  const ports =
    (v.ports ?? [])
      .map((p) => ({
        ...
        targetPort: String(p.targetPort ?? "").trim() || undefined,
...
export function buildPVCYaml(v: PersistentVolumeClaimFormValues): string {
  const obj: any = {
    ...
    spec: {
      ...
      resources: { requests: { storage: v.requestStorage } },
...
export function ServicePortsFormList() {
  return (
        ...
        <Space direction="vertical" style={{ width: "100%" }}>
          {fields.map((f) => (
            <Space key={f.key} style={{ display: "flex" }} align="baseline">
              ...
              <Form.Item name={[f.name, "targetPort"]} style={{ marginBottom: 0 }}>
